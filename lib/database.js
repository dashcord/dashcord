...
function norm(type, rec) {
  ...
  switch (type) {
    ...
    case 'bot':
      ...
      for (const page of rec.pages) if (!page.comps) page.comps = [];
      break;
  }
  return rec;
}
...
class GuildDatabase {
  ...
  async _load(id) {
    const store = await open(`guilddata/${id}.db`);
...
module.exports = {masters, guildDb, initDatabase, generateToken, newBotRecord, updateBot, norm};